...
const ClubApplicationsListing = () => {
  ...
  const handleAction = async (action, row) => {
    if (action === "edit") {
      navigate(`/club-application-form/${row._id}`, { state: { mode: "edit" } });
    }
    if (action === "view") {
      navigate(`/club-application-form/${row._id}`, { state: { mode: "view" } });
    }
    if (action === "delete") {
      ...
      try {
        ...
        await axios.delete(
          `${process.env.REACT_APP_BASE_URL}/join-club-applications/delete/${row._id}`,
          { headers: { Authorization: `${token}` } }
        );
        setApplications((prev) => prev.filter((a) => a._id !== row._id));
      } catch (err) {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await axios.get(`${process.env.REACT_APP_BASE_URL}/club-applications/GetAll`, {
          headers: { Authorization: `${token}` },
        });
  ...
  const COLUMNS = useMemo(
    () => [
      ...
      { Header: "Availability (hrs/week)", accessor: "availability" },
      { Header: "Previous Experience", accessor: "previousExperience" },
      { Header: "Custom Questions", accessor: "customQuestions" },
      {
        Header: "Created At",
        accessor: "createdAt",
        Cell: (row) => (
    ...
    <div>
      <div className="flex justify-end mb-4">
        <Button
          ...
          type="button"
          onClick={() => navigate("/club-application-form/add")}
        />
      </div>

      <Card noborder>
        <div className="md:flex justify-between items-center mb-6">
          <h4 className="card-title">Join Club Applications</h4>
        ...
        <div className="md:flex justify-between items-center mt-6">
          <span className="text-sm font-medium text-slate-600 dark:text-slate-300">
            Page {page} | Total {applications.length} applications
          </span>
...
export default ClubApplicationsListing;
